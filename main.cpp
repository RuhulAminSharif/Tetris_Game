...
// Function to draw the outline of a rectangle for the Ghost piece
void plotRectangleOutline(ll posX, ll posY, ll width, ll height, const Color &colorObj)
{
    glColor4f(colorObj.r, colorObj.g, colorObj.b, 0.8f); // Slightly transparent
    glLineWidth(2.0f);
...
public:
    ...
    // Draw the hollow outline for the Ghost Piece
    void drawOutline(ll offSetX, ll offSetY)
    {
        vector<Position> tiles = getCellPositions();
        for (Position item : tiles)
        {
            plotRectangleOutline(item.col * cellsize + offSetX, item.row * cellsize + offSetY, cellsize - 1, cellsize - 1, colors[id]);
        }
    }
...
public:
    ...
    // Attributes for pause and hold functionalities
    bool isPaused;
    ll holdBlockId;
    bool canHold;
    Game()
    ...
    /// Helper function to instantiate blocks by ID for holding
    Block getBlockById(ll id)
    {
        switch (id)
        {
        case 1:
        ...
        case 7:
            return ZBlock();
        case 8:
            return ComplexBlock1();
        case 9:
            return ComplexBlock2();
        case 10:
            return ComplexBlock3();
        default:
            return LBlock();
        }
    }
    void increaseLevel()
    {
        ...
        gameOver = false;
        levelUpMsg = true;
        holdBlockId = -1; // Clear hold block on level up
    }
    void reset()
    {
    ...
    // Drawing the Ghost block to show projection
    void drawGhost()
    {
        Block ghost = currBlock;
        while (true)
        {
            ghost.move(1, 0);
            bool fits = true;
            ...
            for (Position item : tiles)
            {
                if (grid.isCellOutside(item.row, item.col) || !grid.isCellEmpty(item.row, item.col))
                {
                    fits = false;
                    break;
                }
        ...
        ghost.drawOutline(11, 11);
    }
    void draw()
    {
        grid.draw();
        drawGhost(); // Draw ghost underneath the active block
        ...
        if (nextBlock.id == 3)
            ...
            nextBlock.draw(255, 210);
        else
            nextBlock.draw(270, 210);

        // Draw Hold Block
        if (holdBlockId != -1)
        {
            Block held = getBlockById(holdBlockId);
            if (held.id == 3)
                held.draw(255, 425);
            else if (held.id == 4)
                held.draw(255, 415);
    ...
    void holdPiece()
    {
        if (gameOver || isPaused || !canHold)
            return;

        if (holdBlockId == -1)
        {
            holdBlockId = currBlock.id;
            currBlock = nextBlock;
            nextBlock = getRandomBlock();
        }
        else
        {
            ll tempId = holdBlockId;
            holdBlockId = currBlock.id;
            currBlock = getBlockById(tempId);
        }
        canHold = false;
    }
    ...
    void lockBlock()
    {
        ...
        updateScore(lines, 0);
        canHold = true; // Re-enable holding piece on next turn
    }
    void rotateBlock()
    {
        if (gameOver == false)
...
void update(int value)
{
    ...
    // Only move block down automatically if NOT paused
    if (!myGame.isPaused && !myGame.gameOver)
    {
...
void display(void)
{
    ...
    renderBitmapString(370.0f, 155.0f, GLUT_BITMAP_HELVETICA_18, "Next");

    // Hold UI rendering
    renderBitmapString(370.0f, 345.0f, GLUT_BITMAP_HELVETICA_18, "Hold");

    // Draw Pause Symbol (||) and Indicator Text
    Color white(1.0f, 1.0f, 1.0f, 1.0f);
    plotRectangle(460, 12, 4, 15, white);
    plotRectangle(468, 12, 4, 15, white);
    renderBitmapString(420.0f, 24.0f, GLUT_BITMAP_HELVETICA_12, "Pause");

    if (myGame.gameOver)
    {
        renderBitmapString(340.0f, 540.0f, GLUT_BITMAP_HELVETICA_18, "GAME OVER");
    }

    // Rendering the "PAUSED" text
    if (myGame.isPaused && !myGame.gameOver)
        renderBitmapString(355.0f, 540.0f, GLUT_BITMAP_HELVETICA_18, "PAUSED");
    ...
    {
        renderBitmapString(340.0f, 540.0f, GLUT_BITMAP_HELVETICA_18, "LEVEL UP!");
    ...
    // Scaled down the Next box panel
    DrawRectangleRounded(320.0f, 175.0f, 170.0f, 130.0f, 0.3f, 6, lightBlue);

    // Holding panel
    DrawRectangleRounded(320.0f, 365.0f, 170.0f, 130.0f, 0.3f, 6, lightBlue);
